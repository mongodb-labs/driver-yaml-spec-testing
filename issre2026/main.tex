...
\subsection{Effect on nonconformance bugs}

To measure whether YAML spec tests reduce nonconformance bugs, we mined 13,153 resolved Jira tickets and classified each using an LLM (Claude Sonnet; F1=0.71 for the nonconformance category against a 200-ticket gold corpus), followed by a manual review. We analyzed the 9 drivers (C, C\#, C++, Java, Node.js, Perl, PHP, Python, Ruby) with at least 36~months of pre-spec ticket history. Fig.~\ref{fig:crud_bugs} shows CRUD nonconformance bugs per year. The CRUD prose specification and its first YAML tests were both published in 2015. Bugs filed before then can be correctly classified as nonconformance bugs, although there was no written spec. E.g., if drivers should send a \texttt{\$maxScan} key to the server but one sends \texttt{\$maxscan} instead (with a lowercase ``s''), this behavior is nonconformance to the unwritten spec. Bugs spiked in 2015 as the new spec and tests revealed nonconformance across all drivers. From 2016 onward, fewer new bugs were filed than before the tests. A bump in 2019 (17 bugs) reflects coverage gaps in the YAML test corpus: several bugs involved bulk write edge cases that were not YAML-tested, or multi-server failure paths that are difficult to express declaratively.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{FIG/crud_spike_decay_balanced.pdf}
    \caption{CRUD spec nonconformance bugs per year for 9 drivers. The CRUD spec and YAML tests were published in 2015.}
    \label{fig:crud_bugs}
\end{figure}
